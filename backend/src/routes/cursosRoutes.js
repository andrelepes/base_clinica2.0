...
cursosRoutes.use(ensureAuthenticated);
...
cursosRoutes.get('/data', (req, res) => {
  console.log('Filtrando cursos por data');
  const { dataInicio, dataFim } = req.query;
  db.any('SELECT * FROM cursos WHERE data_conclusao BETWEEN $1 AND $2', [
    dataInicio,
    dataFim,
  ])
    .then((cursos) => {
...
cursosRoutes.get('/horas', (req, res) => {
  console.log('Filtrando cursos por horas');
  const { minHoras, maxHoras } = req.query;
  db.any('SELECT * FROM cursos WHERE horas_curso BETWEEN $1 AND $2', [
    minHoras,
    maxHoras,
  ])
    .then((cursos) => {
...
cursosRoutes.get('/busca', (req, res) => {
  console.log('Buscando cursos por termo');
  const { termo } = req.query;
  db.any('SELECT * FROM cursos WHERE titulo ILIKE $1 OR descricao ILIKE $1', [
    `%${termo}%`,
  ])
    .then((cursos) => {
      res.json(cursos);
...
cursosRoutes.post(
  ...
  [
    ...
    check('horas_curso')
      .isInt({ min: 1 })
      .withMessage('Horas do curso deve ser um número inteiro maior que 0'),
  ],
  (req, res) => {
    ...
    db.none(
      'INSERT INTO cursos(titulo, descricao, data_conclusao, horas_curso) VALUES($1, $2, $3, $4)',
      [titulo, descricao, data_conclusao, horas_curso]
    )
      .then(() => {
...
cursosRoutes.put(
  ...
  (req, res) => {
    ...
    db.none(
      'UPDATE cursos SET titulo = $1, descricao = $2, data_conclusao = $3, horas_curso = $4 WHERE id = $5',
      [titulo, descricao, data_conclusao, horas_curso, cursoId]
    )
      .then(() => {
...
// Exportando o cursosRoutes no final do arquivo
